// (Type imports removed: ChromaticNote, ScaleMode, ChordProgression, Pitch)
...
/**
 * @typedef {Object} ProgressionOptions
 * @property {number} [length] - Progression length
 * @property {number} [octave] - Octave
 * @property {'triad'|'seventh'|'extended'} [voicing] - Chord voicing
 */
    ...
    progression.chords.forEach((_, chordIndex) => {
      ...
      if (strumPattern) {
        ...
        chordPitches.forEach((pitch, noteIndex) => {
          notes.push({
              pitch: JMonConverter.midiToNoteName(pitch),
        ...
        // Play chord as block
        notes.push({
            pitch: chordPitches.map(pitch => JMonConverter.midiToNoteName(pitch)),
