// (Type imports removed: Note, Sequence, Pitch, Duration, RhythmPattern)
// (Type import removed: JMonNote, JMonSequence, JMonComposition, BasicJMonComposition, MusicalTime, NoteDuration, AudioNode)
...
export class JMonConverter {
   ...
   */
  static rhythmPatternToJMonSequence(
    pattern,
    pitches = [60], // Default to middle C
  ...
  composition.sequences?.forEach((seq, seqIdx) => {
      if (!seq.label) {
        errors.push(`Sequence ${seqIdx} missing label`);
      }

      if (!seq.notes || seq.notes.length === 0) {
        errors.push(`Sequence ${seqIdx} has no notes`);
      }

      // Validation glissando/pitchbend
  seq.notes?.forEach((note, noteIdx) => {
        const hasGliss = note.articulation === 'glissando' && typeof note.glissTarget === 'number';
        const pitchBends = Array.isArray(note.modulations)
          ? note.modulations.filter(m => m.type === 'pitchBend')
          : [];

        if (hasGliss && pitchBends.length > 0) {
          // Vérifie cohérence : pitchbend doit aller de note.note à note.glissTarget
          const startPB = pitchBends[0]?.value;
          const endPB = pitchBends[pitchBends.length - 1]?.value;
          // On ne peut pas deviner la plage exacte, mais on peut signaler la présence des deux
          if (startPB === undefined || endPB === undefined) {
            warnings.push(`Note ${noteIdx} in sequence ${seqIdx}: glissando and pitchBend both present, but pitchBend values are incomplete.`);
          } else {
            // Optionnel : on pourrait vérifier la correspondance exacte si la plage de pitchbend est connue
            warnings.push(`Note ${noteIdx} in sequence ${seqIdx}: both glissando and pitchBend present. Ensure they are consistent.`);
          }
        } else if (hasGliss && pitchBends.length === 0) {
          warnings.push(`Note ${noteIdx} in sequence ${seqIdx}: glissando present but no pitchBend modulation. A converter should generate pitchBend for playback.`);
        } else if (!hasGliss && pitchBends.length > 0) {
          warnings.push(`Note ${noteIdx} in sequence ${seqIdx}: pitchBend modulation present but no glissando articulation. Consider adding articulation: 'glissando' and glissTarget.`);
        }
      });
    });
