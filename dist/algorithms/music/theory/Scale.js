...
const MusicTheoryConstants_1 = require("./MusicTheoryConstants");
const conversion_1 = require("../../io/jmon/conversion");
class Scale {
    ...
    generate(octave = 4, length) {
        const intervals = MusicTheoryConstants_1.MusicTheoryConstants.scaleIntervals[this.mode];
        const tonicIndex = MusicTheoryConstants_1.MusicTheoryConstants.getChromaticIndex(this.tonic);
    ...
    getNoteNames() {
        ...
        return intervals.map(interval => {
            const noteIndex = (tonicIndex + interval) % 12;
            return MusicTheoryConstants_1.MusicTheoryConstants.chromaticScale[noteIndex];
        });
    }
    isInScale(pitch) {
    ...
    toJMonSequence(options = {}) {
        ...
        const notes = pitches.map((pitch, index) => ({
            note: conversion_1.JMonConverter.midiToNoteName(pitch),
            time: conversion_1.JMonConverter.timeToMusicalTime(index),
            duration,
            velocity
        }));
        return {
